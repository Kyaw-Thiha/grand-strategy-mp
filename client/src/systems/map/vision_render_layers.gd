...
## below province fills' implicit z=0 and FOG_OVERLAY_Z, avoiding a z-index tie with
## province-level rendering); the border Node2D is added as a later sibling of the fill
## Node2D in SubprovinceRenderer, so within this shared layer borders still draw above
## fills via canvas-item tree order.
