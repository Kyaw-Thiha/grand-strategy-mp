...
func create_lobby() -> void:
	...
	if result.is_empty() or result.has("error"):
		lobby_join_failed.emit(result.get("error", "Failed to create lobby"))
		return

	_current_join_code = result.get("join_code", "")
...
func join_by_code(code: String) -> void:
	...
	if result.is_empty() or not result.has("room_id"):
		lobby_join_failed.emit(result.get("error", "Invalid join code"))
		return

	var room_id: String = result.get("room_id", "")
...
func join_public_game() -> void:
	var result: Variant = await APIClient.get_req("/lobby/public")
	if not result is Array or result.is_empty():
