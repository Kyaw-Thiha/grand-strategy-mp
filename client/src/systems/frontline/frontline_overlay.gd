...
var _divisions: Dictionary = {}  # div_id → { lng, lat, nation_id, hp }
...
func _on_division_changed(div_id: String) -> void:
	...
	if data.is_empty() or data.get("combat_state", "") == "destroyed":
		...
		_divisions[div_id] = {
			"lng":       float(data.get("position_lng", 0.0)),
			"lat":       float(data.get("position_lat", 0.0)),
...
func _draw() -> void:
	...
	for div_id: String in _divisions:
		...
		var pos: Vector2 = _map_loader.project_lng_lat(d["lng"], d["lat"])
