...
**Deprecated.** `autoAssignEscort()`'s round-robin pairing was retired — Escort is now driven entirely through `AirMissionTargetingSystem`'s per-tick tier chain, the same as every other mission (see "Mission auto-targeting" below and "Escort formation flying" further down). Only a comment remains at `air_wing_lifecycle_system.ts` explaining the retirement; the function itself no longer exists.
...
**Current.** `AirMissionTargetingSystem` (`game-server/src/systems/air_mission_targeting.ts`) drives per-tick target selection for every retargetable (`IDLE`/`LOITER`/`TRANSIT`) wing that isn't `IDLE`-missioned — including `ESCORT`, which has its own full tier chain (`resolveEscortTargets`, `RECON_ESCORT_BOMBER_TYPES`/`HEAVY_FIGHTER_ESCORT_PRIMARY`/`FALLBACK`, `buildEscortCounts`) resolved and committed the same way as every other mission. It runs in `GameRoom.gameTick()` after `AirWingLifecycleSystem.tick()` (so this tick's LOITER/IDLE transitions are visible) and before the RTB/Dubins path ticks (so a freshly-committed TRANSIT wing gets its path advanced the same tick). See `AIR_COMBAT.md`'s "Mission Auto-Targeting & Patrol Priority" section for the authoritative per-mission tier chains, crowd-balancing formula, and hysteresis rule this module implements — this note only summarizes the implementation shape.

- Each mission (Interception, Air Superiority, Tactical Bombing, Strategic Bombing sub-missions, Naval sub-missions, Escort, Recon) has its own exported tier-chain resolver function; `AirMissionTargetingSystem._resolveForMission` dispatches on `wing.mission` (Escort is dispatched separately, since committing to a bomber also sets the rendezvous fields described below).
...
## Escort and recon formation flying

**Current.** `air_dubins_pathfinder.ts`'s `tick()` has a dedicated per-tick loop (separate from the generic Dubins path advancement) that flies Escort and Recon wings relative to their assigned bomber's live position, rather than a one-shot path built at mission-commit time.

- **Escort ahead/loiter.** An escort screens slightly ahead of its bomber along the bomber's heading. Once it's within one tick's travel of that ahead point, it snaps directly there and enters `LOITER` on a tight station-keeping circle (`computeLoiterArc`), holding until the bomber's forward motion pulls the ahead point far enough away to resume closing. The ahead point is computed from a *stable* heading, not the bomber's raw `heading_deg` — if the bomber is itself self-loitering (heading rotating continuously around its own circle), the escort instead uses `_lastTransitHeading`, the bomber's last heading while genuinely in `TRANSIT`. Without this, the escort's ahead point orbits with the bomber's loiter and combines with the loiter arc's fixed turn direction to produce a repeatable one-sided drift.
- **Break-off intercept.** An escort abandons its ahead/loiter station to chase a hostile wing that's closing on its bomber (`escort_intercept_id`, triggered within `BREAKOFF_TRIGGER_RANGE_DEG`, abandoned past `BREAKOFF_ABANDON_RANGE_DEG` or once the threat is no longer pursuing the bomber), then resumes formation afterward.
- **Rendezvous.** When an escort commits to a *new* bomber (`air_mission_targeting.ts`'s escort-commit branch), the bomber gets `AirWingState.escorted_by_wing_id` set to the escort's wing ID and `awaiting_escort_rendezvous = true`. While that flag is set, the bomber's per-tick path advancement is throttled to `RENDEZVOUS_SLOWDOWN_FACTOR` (default 0.3×) instead of a hard freeze, giving a newly-assigned escort a real chance to catch up and get ahead instead of perpetually trailing. `air_wing_lifecycle_system.ts` clears the flag once the escort reaches `LOITER` (has formed up), is reassigned elsewhere, is lost, or `RENDEZVOUS_TIMEOUT_TICKS` elapses as a safety valve.
- **Recon trail-following.** A recon wing on its tier-1 "follow a friendly bomber" target trails behind it using a lead-projected anchor (`_leadFormationPoint`) rather than a fixed offset, so the trail point stays ahead of where the bomber will be rather than perpetually chasing its current position. `_repathIfDrifted` only rebuilds the wing's path when it's expired or the anchor has drifted past `REPATH_DRIFT_THRESHOLD_DEG` — holding a still-valid path across several ticks instead of rebuilding (and resetting client interpolation) every tick. The rebuilt path's geometric length is padded to comfortably outlive the expected interval until the next drift-triggered rebuild (derived from the bomber's speed and `REPATH_DRIFT_THRESHOLD_DEG`); without this, a recon trailing a slower bomber would rebuild before its previous path's travel time had elapsed, leaving the wing's server-side position clamped at the old path's endpoint for part of every cycle.
