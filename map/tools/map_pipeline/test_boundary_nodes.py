...
def test_boundary_nodes_appear_at_cover_transition():
    ...
    sources = {
        "cover": [
            _cover_feat([(0,-2),(1,-2),(1,2),(0,2),(0,-2)], "plains"),
            _cover_feat([(1,-2),(2,-2),(2,2),(1,2),(1,-2)], "dense_forest"),
        ],
        "elevation": [],
        "base_water": [],
        "provinces": [],
    }
    nodes, edges = insert_boundary_nodes(sources, _empty_wp(), boundary_sample_deg=0.2)
...
def test_boundary_nodes_scale_with_boundary_length_not_area():
    ...
    n1, _ = insert_boundary_nodes(_sources(0.5),  _empty_wp(), boundary_sample_deg=0.2)
    n2, _ = insert_boundary_nodes(_sources(2.0),  _empty_wp(), boundary_sample_deg=0.2)
...
def test_boundary_nodes_connect_to_existing_nodes():
    ...
    nodes, edges = insert_boundary_nodes(sources, existing_wp, boundary_sample_deg=0.2)
    if nodes:
...
def test_nation_id_on_boundary_nodes():
    ...
    nodes, _ = insert_boundary_nodes(sources, _empty_wp(), boundary_sample_deg=0.2)
    assert len(nodes) > 0
    for n in nodes:
...
def test_dissolve_scales_with_type_count_not_polygon_count():
    """10 alternating-type polys must use type-pair sampling, not polygon-pair."""
    import time
    feats = []
    for i in range(5):
        x0 = i * 0.20
        ...
        feats.append(_cover_feat([(x0+0.10,0),(x1+0.10,0),(x1+0.10,1),(x0+0.10,1),(x0+0.10,0)], "dense_forest"))
    sources = {"cover": feats, "elevation": [], "base_water": [], "provinces": []}
    t0 = time.time()
    nodes, edges = insert_boundary_nodes(sources, _empty_wp())
    elapsed = time.time() - t0
    assert elapsed < 10.0, f"Dissolve took {elapsed:.1f}s — likely still using polygon-pair iteration"
    assert len(nodes) > 0
